...
proc sort data=adam.adsl out=adsl;
  by usubjid;
  where upcase(strip(mittfl))="Y" and trt01an ne .;
run;
...
data &var.;
 ...
 where &var. ne .;
run;
proc means data=&var. noprint maxdec=0;
 class trt01an;
 var &var.;
